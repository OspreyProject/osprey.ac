...
|---------------------------------|---------|---------|------------------------------------------------------------------------------------------------------------------------|
| `HideWarningProceedButton`           | boolean | false   | If true, the warning page proceed button is hidden.                                                                 |
| `HideWarningReportButton`              | boolean | false   | If true, the warning page report button is hidden.                                                                     |
| `HideProviderControls`         | boolean | false   | If true, provider controls are hidden from the popup and options page.                                                |
| `LockProviderSettings`         | boolean | false   | If true, provider toggles and provider settings are locked in the options page.                                        |
| `LockUserAllowlist`   | boolean | false   | If true, the user cannot edit or clear the allowed websites list, but existing entries still apply.                                                                |
| `DisableSettingsReset`           | boolean | false   | If true, reset actions in the settings page are disabled.                                                              |
| `DisableThirdPartyProviders` | boolean | false   | If true, third-party providers are disabled and cannot be enabled from settings.                             |
...
| `ManagedProviderSettings` | object           | (empty object) | Per-provider overrides keyed by provider id. Each value may set "enabled" (boolean), "bypassBlockingThreshold" (boolean), "requestTimeoutMs" (integer between 1000 and 60000), and "blockCategories" (an object mapping a category key to a boolean). |
...
| `BrandName`   | string | (empty) | Name shown in the warning page banner and popup, replacing the default Osprey product name. When empty, the default name is shown.                                                    |
...
| `CustomWarningMessage` | string | (empty) | Optional override for the lead sentence on the warning page. When empty, the default warning message is shown.                                                                          |
...
- `ManagedProviderSettings` is keyed by provider id (for example `alphamountain`, `cloudflare`, `urlhaus`). Each entry
  may set `enabled` (boolean, the managed enabled state for the provider), `bypassBlockingThreshold` (boolean),
  `requestTimeoutMs` (integer, 1000 to 60000), and `blockCategories` (an object mapping a provider-specific category
  key such as `suspicious` or `newly_registered` to a boolean).
